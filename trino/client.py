...
class TrinoRequest(object):
    ...
    def http_headers(self) -> Dict[str, str]:
        headers = requests.structures.CaseInsensitiveDict(
            {
                ...
                constants.HEADER_TIMEZONE: self._client_session.timezone,
                constants.HEADER_CLIENT_CAPABILITIES: "PARAMETRIC_DATETIME",
                "user-agent": f"{constants.CLIENT_NAME}/{__version__}",
            }
        )

        roles = self._client_session.roles
        if roles:
        ...
        client_tags = self._client_session.client_tags
        if client_tags:
            headers[constants.HEADER_CLIENT_TAGS] = ",".join(client_tags)
        ...
        prepared_statements = self._client_session.prepared_statements
        ...
        extra_credential = self._client_session.extra_credential
        if extra_credential:
            for tup in extra_credential:
    ...
    def post(
        ...
        http_headers = ChainMap(additional_http_headers or {}, self.http_headers)
